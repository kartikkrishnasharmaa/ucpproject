    ...
    <div className="absolute inset-0 overflow-hidden z-0">
      {Array.from({ length: 20 }).map((_, i) => {
        ...
        return (
          <motion.div
            key={i}
            className="absolute text-black text-sm sm:text-base lg:text-lg opacity-50 pointer-events-none"
            style={{ left: `${left}%`, top: `${Math.random() * 100}%` }}
            animate={{ y: ['100%', '-100%'] }}
  ...
  return (
    <footer className="relative overflow-hidden text-blue-900 body-font z-10">
      ...
      <div className="container py-24 flex md:items-center lg:items-start md:flex-row md:flex-nowrap flex-wrap flex-col relative z-10">
        <div className="flex-grow flex-wrap md:pr-20 lg:ml-80 md:text-left text-left">
          <div className="lg:w-2/4 md:w-1/2 w-full px-4 ml-5">
            <h2 className="title-font font-bold text-white-300 tracking-widest text-sm mb-3">UnstopComputer</h2>
            <nav className="list-none mb-10 space-y-2">
              <ul className="space-y-1">
                <li><Link href="/main/about" className="text-blue-400 hover:text-white transition">About us</Link></li>
                <li><Link href="/main/advertise" className="text-blue-400 hover:text-white transition">Advertise with us</Link></li>
                <li><Link href="/main/career" className="text-blue-400 hover:text-white transition">Career</Link></li>
                <li><Link href="/main/contact" className="text-blue-400 hover:text-white transition">Contact us</Link></li>
                <li><Link href="/main/donate" className="text-blue-400 hover:text-white transition">Support Us</Link></li>
                <li><Link href="/main/privacy-policy" className="text-blue-400 hover:text-white transition">Privacy Policy</Link></li>
                <li><Link href="/main/terms-and-condition" className="text-blue-400 hover:text-white transition">Terms Condition</Link></li>
              </ul>
            </nav>
          </div>
        </div>
      ...
      <b className="mt-6 float-right text-white mr-2 sm:text-center relative z-10">
        Site Last updated: <span className="font-normal">{formatLastUpdated(lastUpdated)}</span>
      </b>
